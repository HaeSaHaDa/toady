...
public class UserCustomDetails implements UserDetails, OAuth2User {
	 ...
	 * ArrayList<>(); collectors.add(new GrantedAuthority() { //자바는 메서드를 add에 못 넣지만
	 * 객체는 넣을 수 있어. //그런데 GrantedAuthority()는 메서드가 getAuthority()하나야 //굳이 메서드를 한 번
	 * 감싸야 해. 그래서 자바 1.8부터는 람다식을 지
	 * 
	 * @Override public String getAuthority() {//new GrantedAuthority() 할 때 오버라이딩.
	 * return "ROLE_" + user.getRole(); //스프링에서 롤을 받을 때 규칙. ROLE_USER. //그냥 ROLE이라고
	 * 하면 인식을 못해. } }); return collectors; }
	 */
}
